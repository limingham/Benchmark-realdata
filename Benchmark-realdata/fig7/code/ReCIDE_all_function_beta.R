...
SecondFC_filter <- function(SC_ref_sub,n_cores=5){
  ...
  patient_id<-patient_id[patient_id[,2]>50,]
  ##??????sign????????????
  func_in<-function(i){
...
# 
# results_deconvolution = sep_solDWLS
# method='DWLS'
...
ReCIDE_PCA=function(results_deconvolution,method='DWLS'){
  ...
  func_pca<- function(n){
    ...
    if(ncol(data_train)>=3){
      ...
      # if( n ==4){pca_n = pca_n[2:nrow(pca_n),]}
  ...
  prd_df2<-pbmclapply(1:length(prd_df),func_pca,mc.cores=20)
