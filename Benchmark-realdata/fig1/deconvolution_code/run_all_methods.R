...
func_run_CIBERSORT = function(combined.data,bulk.mtx,celltype,subject,dir_ref,dir_results){
  ...
  patient_id_df<-patient_id_df[patient_id_df[,2]>50,]
...
func_run_music = function(ref_seurat,EXP_df,celltype,subject,dir_results){
  ...
  # bulk.mtx<-EXP_df
  inter_rowname = intersect(row.names(bulk.mtx),row.names(ref_seurat))
  bulk.mtx = bulk.mtx[inter_rowname,]
  ref_seurat = ref_seurat[inter_rowname,]
  ...
  music_df = music_output[["Est.prop.weighted"]]
  
  music_df2 = music_output[["Est.prop.allgene"]]
  
  saveRDS(music_df,file= dir_results)
  # saveRDS(music_output,file= paste0(dir_results,'music_all.rds'))
  saveRDS(music_output,file= paste0(dir_results,'music_allallgene.rds'))
...
func_run_bayes = function(ref_seurat,bulk.mtx,subject ='1',celltype,dir_results,key1=NULL){
  kk<-as.data.frame(ref_seurat@assays[["RNA"]]@counts)
  kksum<-as.data.frame(apply(kk, 2, sum))
  all(kksum>0)
  kksum<-subset(kksum,kksum[,1]>0,)
  # kksum<-subset(kksum,kksum[,1]>100,)
  ...
  cell.state.labels=as.character(cell.type.labels)
  if(!(is.null(key1))){
  subject_in = ref_seurat@meta.data[[subject]]
  for (mm in 1:length(cell.state.labels)) {
    if(cell.state.labels[mm] == key1){
      cell.state.labels[mm] = paste0(subject_in[mm],cell.state.labels[mm])
      }
